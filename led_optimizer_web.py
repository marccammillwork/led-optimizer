...
def optimized_allocation(runs, opts, max_connections):
    ...
    while runs_left:
        ...
        if best_pair:
            ...
            length, cost = (min(candidates, key=lambda x: x[1]) if candidates else max(opts.items(), key=lambda x: x[0]))
            allocations.append(
                {
                    "strip_length": length,
                    "used": (r,),
...
def compute_power(allocations):
    ...
    for load in sorted(segment_watts, reverse=True):
        ...
        if placed:
            continue
        spec = next((s for s in power_specs if s["W"] >= load * 1.2), None) or next(
            (s for s in power_specs if s["W"] >= load), power_specs[-1]
        )
        bins.append(
            {
                "W": spec["W"],
...
if st.button("Optimize All Orders"):
    ...
    for od in order_details:
        led_cost = sum(i["cost"] for i in od["alloc"])
    ...
    for od in order_details:
        with st.expander(f"Order {od['order']}"):
            df_o_disp = pd.DataFrame(od["alloc"])
    ...
    with zipfile.ZipFile(buf, "w") as zf:
        for od in order_details:
            ...
            # "Excel" (actually CSV for portability)
    ...
    buf.seek(0)
    st.download_button("Export Data", data=buf.getvalue(), file_name=f"{
