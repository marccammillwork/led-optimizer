...
st.subheader("Enter order number and runs (inches), then click Optimize")
...
# Clean data: blanks remain blank
...
if st.button("Optimize All Orders"):
    ...
    # Format Cost with $ and hide zeros
    df_rolls['Cost'] = df_rolls['Cost'].apply(lambda x: f"${x:.2f}")
    df_rolls.loc[df_rolls['Cost'] == "$0.00", 'Cost'] = ""
    ...
    df_ps_disp.loc[df_ps_disp['Cost'] == "$0.00", 'Cost'] = ""
    ...
    for od in orders:
        ...
        with st.expander(f"Order {od['order']}"):
            ...
            ps_o, cost_o, sup_counts = compute_power(od['alloc'])
            ps_o_disp = ps_o.drop(columns=['Supply #']).copy()
            ps_o_disp['Cost'] = ps_o_disp['Cost'].apply(lambda x: f"${x:.2f}")
            ps_o_disp['Remaining (W)'] = ps_o_disp['Remaining (W)'].apply(lambda x: f"{x}")
            ps_o_disp['Loads (W)'] = ps_o_disp['Loads (W)'].apply(lambda x: str(x))
            # Hide zeros/empty
            ps_o_disp = ps_o_disp.replace({0: "", "": ""})
